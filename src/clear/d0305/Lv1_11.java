...
public class Lv1_11 {
    public String[] solution(String[] players, String[] callings) {
    	...
    	for (int i = 0; i < players.length; i++) map.put(players[i], i); // 값 저장
    	
    	for (String call : callings) {
			int idx = map.get(call); // call나온 값 map 위치 찾음
			
			if(idx>0) {
				String temp = players[idx-1]; // 값 서로 바꿈
				players[idx-1] = players[idx];
				players[idx] = temp;
				
				map.put(players[idx], idx); // map에 적용(다음번에도 찾기 위해)
