...
public class Lv0_131 {
    public int solution(int[] date1, int[] date2) {
		int answer = 0;	
    	if(date1[0] < date2[0]) answer= 1; // < 작은게 빠른거
    	else if(date1[0] == date2[0] && date1[1] < date2[1]) answer = 1; // 년(앞의 것)은 같아야 함
    	else if(date1[0] == date2[0] && date1[1] == date2[1] && date1[2] < date2[2]) answer = 1;
    	return answer;
    }
}

// 문제 요약 : 두 배열은 [year,month,day]꼴 date1>date ? 1 : 0

// 날짜는 LocalDate 사용하면 편함
