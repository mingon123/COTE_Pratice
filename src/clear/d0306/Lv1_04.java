...
public class Lv1_04 {
    public String solution(String video_len, String pos, String op_start, String op_end, String[] commands) {
    	int videoLen = getTime(video_len); // 전부 초로 바꿈
    	int postime = getTime(pos);
    	int opStart = getTime(op_start);
    	int opEnd = getTime(op_end);

    	for (int i = 0; i < commands.length; i++) {
        	if(opStart <= postime && postime <= opEnd) postime = opEnd; // skipOp
        	
			if(commands[i].equals("prev")) { // 앞으로 이동
				if(postime<10) postime = 0;
				else postime-=10;
			} else if(commands[i].equals("next")) { // 뒤로 이동
...
// -10sec, +10sec, skipOp
// -10sec : prev 입력 시, 10초미만이면 처음으로 -> 0분0초
// +10sec : next 입력 시, 10초미만이면 마지막으로 -> vedio_len 
// skipOp : op_start <= 지금 <= op_end => op_end로 이동
// vedio_len:동영상 길이, pos:재생위치, op_start:오프닝시작, op_end:오프닝끝, commands:사용자 입력
// 모든 입력 끝난 후 동영상 위치 출력 "mm:ss"형식
// 한자리면 0을 붙여 2자리로 출력
// 순서 : 입력 다하고 마지막 오프닝이면 오프닝 끝으로 이동