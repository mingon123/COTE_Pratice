...
public class Lv1_03 {
    public int solution(int[] schedules, int[][] timelogs, int startday) {
    	...
    	for (int i = 0; i < schedules.length; i++) {
    		int late;
    		if(schedules[i]%100 < 50) late = schedules[i] + 10; // 50분 미만이면 10분 더함	
    		else late = schedules[i] + 50;                      // 50분 이상이면 50분 더함 10:55분이면 1105-1055 = 50
  
			boolean l = true;
			int day = startday;
			for (int j = 0; j < 7; j++) { // 모든 날짜 반복
				if(day==6 || day==7) { // 주말은 하루 더하고 패스
					day = (day%7)+1;
					continue;
				}
				
				if(timelogs[i][j] > late) { // 평일에 하루라도 지각하면 거짓
					l = false;
					break;
				}
				day = (day%7)+1; // 각 날짜 1234567뜨도록
			} // for j
			if(l) count++; // 참이면 ++
		} // for i
    	return count;
