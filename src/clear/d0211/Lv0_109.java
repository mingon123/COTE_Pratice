...
public class Lv0_109 {
	public int[] solution(int[] arr, boolean[] flag) {
		List<Integer> X = new ArrayList<>();
		for (int i = 0; i < flag.length; i++) {
			if(flag[i]==true) { // 참이면
				for (int j = 0; j < arr[i]*2; j++) X.add(arr[i]); // arr[i]*2까지 arr[i]를 추가
			}
			else {
				for (int j = 0; j < arr[i]; j++) X.remove(X.size()-1); // false면 마지막 값 arr[i]까지 제거
			}
		} // for i
...
// 문제 요약 : 빈배열'X', 같은 길이의 arr,flag. flag를 순회하며 flag[i]가 true면 X뒤에 arr[i]를 2번추가,
//	 	 										    flag[i] false면 X에서 마지막 arr[i] 제거 X리턴
